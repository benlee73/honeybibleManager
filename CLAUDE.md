...
- `message_corrections`: 지나간 인증의 오타·누락을 교정한다(`analyzer.apply_message_corrections`). 카톡방의 과거 메시지는 본인이 재인증하지 않으면 고칠 수 없어서, `room`·`user`·`find` 3중으로 좁힌 목록을 둔다. 진도표 필터보다 먼저 적용된다. 날짜 오타(`9/11-13`→`6/11-13`)와 이모티콘 누락(`7/15`→`7/15🙈`) 모두 여기서 처리한다. `find`는 부분 문자열이므로, 같은 사람의 다른 메시지에 그 문자열이 또 있으면 이중 치환되니 등록 전에 확인한다.
