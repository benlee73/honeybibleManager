...
class TestParseTxtGalaxy:
    def test_갤럭시_사용자_메시지_파싱(self):
        text = (
            "저장한 날짜 : 2026년 2월 9일 오후 6:35\r\n"
            "2026년 2월 2일 오전 7:52, 김태환 : 2/2🍉\r\n"
        )
        rows = parse_txt(text)
        assert len(rows) == 1
        assert rows[0] == ("김태환", "2/2🍉")

    def test_갤럭시_시스템_메시지_스킵(self):
        text = (
            "저장한 날짜 : 2026년 2월 9일 오후 6:35\r\n"
            "2026년 2월 1일 오후 8:24, 김태환님이 홍길동님을 초대했습니다.\r\n"
            "2026년 2월 2일 오전 7:52, 김태환 : 2/2🍉\r\n"
        )
        rows = parse_txt(text)
        assert len(rows) == 1
    ...
    def test_갤럭시_멀티라인_메시지(self):
        text = (
            "저장한 날짜 : 2026년 2월 9일 오후 6:35\r\n"
            "2026년 2월 1일 오후 8:25, 김태환 : 첫줄\r\n"
            "둘째줄\r\n"
            "셋째줄\r\n"
            "2026년 2월 2일 오전 7:52, 홍길동 : 단일줄\r\n"
        )
        rows = parse_txt(text)
        assert len(rows) == 2
        assert rows[0] == ("김태환", "첫줄\n둘째줄\n셋째줄")
        assert rows[1] == ("홍길동", "단일줄")

    def test_갤럭시_삭제_메시지_처리(self):
        text = (
            "저장한 날짜 : 2026년 2월 9일 오후 6:35\r\n"
            "2026년 2월 2일 오전 8:29, 추찬미 : 2/2 🌈\r\n"
            "메시지가 삭제되었습니다.\r\n"
            "2026년 2월 2일 오전 9:39, 이시현 : 2/2🍊\r\n"
        )
        rows = parse_txt(text)
        assert len(rows) == 2
        # 삭제 메시지는 타임스탬프 없어 이전 사용자의 멀티라인으로 연결됨
        assert rows[0] == ("추찬미", "2/2 🌈\n메시지가 삭제되었습니다.")
        assert rows[1] == ("이시현", "2/2🍊")

    def test_갤럭시_종합_시나리오(self):
        text = (
            "🚩2026 성경일독 PART 1 16 카카오톡 대화\r\n"
            "저장한 날짜 : 2026년 2월 9일 오후 6:35\r\n"
            "\r\n"
            "2026년 2월 1일 오후 8:24\r\n"
            "2026년 2월 1일 오후 8:24, 김태환님이 홍길동님을 초대했습니다.\r\n"
            "2026년 2월 1일 오후 8:24, 김태환 : 안녕하세요\r\n"
            "성경 읽기 방입니다~!\r\n"
            "\r\n"
            "2026년 2월 2일 오전 7:52\r\n"
            "2026년 2월 2일 오전 7:52, 김태환 : 2/2🍉\r\n"
            "2026년 2월 2일 오전 8:28, 황지운 : 2/2 🦭\r\n"
            "메시지가 삭제되었습니다.\r\n"
            "2026년 2월 2일 오전 9:39, 이시현 : 2/2🍊\r\n"
        )
        rows = parse_txt(text)
        assert len(rows) == 4
        assert rows[0] == ("김태환", "안녕하세요\n성경 읽기 방입니다~!")
        assert rows[1] == ("김태환", "2/2🍉")
        assert rows[2] == ("황지운", "2/2 🦭\n메시지가 삭제되었습니다.")
        assert rows[3] == ("이시현", "2/2🍊")


class TestExtractChatMetaGalaxy:
    def test_갤럭시_방이름_및_저장날짜_추출(self):
        text = (
            "🚩2026 성경일독 PART 1 16 카카오톡 대화\r\n"
            "저장한 날짜 : 2026년 2월 9일 오후 6:35\r\n"
        )
        meta = extract_chat_meta(text)
        assert meta["room_name"] == "🚩2026 성경일독 PART 1 16"
        assert meta["saved_date"] == "2026/02/09-18:35"

    def test_갤럭시_오후_시간_변환(self):
        text = (
            "테스트방 카카오톡 대화\r\n"
            "저장한 날짜 : 2026년 2월 10일 오후 3:30\r\n"
        )
        meta = extract_chat_meta(text)
        assert meta["room_name"] == "테스트방"
        assert meta["saved_date"] == "2026/02/10-15:30"

    def test_갤럭시_오후_12시_유지(self):
        text = (
            "테스트방 카카오톡 대화\r\n"
            "저장한 날짜 : 2026년 2월 10일 오후 12:05\r\n"
        )
        meta = extract_chat_meta(text)
        assert meta["saved_date"] == "2026/02/10-12:05"

    def test_갤럭시_오전_12시_0시_변환(self):
        text = (
            "테스트방 카카오톡 대화\r\n"
            "저장한 날짜 : 2026년 1월 1일 오전 12:30\r\n"
        )
        meta = extract_chat_meta(text)
        assert meta["saved_date"] == "2026/01/01-00:30"

    def test_갤럭시_방이름_없음__None_반환(self):
        text = "저장한 날짜 : 2026년 2월 9일 오후 6:35\r\n"
        meta = extract_chat_meta(text)
        assert meta["room_name"] is None
        assert meta["saved_date"] == "2026/02/09-18:35"
