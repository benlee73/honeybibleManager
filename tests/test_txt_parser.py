...
class TestParseTxtWindows:
    def test_윈도우_사용자_메시지_파싱(self):
        ...
        rows = parse_txt(text)
        assert len(rows) == 1
        assert rows[0] == ("김예슬", "안녕하세요!")

    def test_윈도우_시스템_메시지_스킵(self):
        text = (
            "저장한 날짜 : 2026-02-14 14:09:18\r\n"
            "김예슬님이 홍길동님을 초대했습니다.\r\n"
            "[김예슬] [오후 8:35] 안녕하세요!\r\n"
    ...
    def test_윈도우_멀티라인_메시지(self):
        ...
        rows = parse_txt(text)
        assert len(rows) == 2
        assert rows[0] == ("김예슬", "첫줄\n둘째줄\n셋째줄")
        assert rows[1] == ("홍길동", "단일줄")

    def test_윈도우_이모지_사용자명(self):
        text = (
            "저장한 날짜 : 2026-02-14 14:09:18\r\n"
            "[🍯 김신영 99 🦄] [오전 8:44] 김신영 🍡\r\n"
        )
        rows = parse_txt(text)
        assert len(rows) == 1
        assert rows[0] == ("🍯 김신영 99 🦄", "김신영 🍡")

    def test_윈도우_종합_시나리오(self):
        text = (
            "📖26 성경일독 PART1🙏🏻 님과 카카오톡 대화\r\n"
            "저장한 날짜 : 2026-02-14 14:09:18\r\n"
            "\r\n"
            "--------------- 2026년 2월 1일 일요일 ---------------\r\n"
            "김예슬님이 홍길동님을 초대했습니다.\r\n"
            "김예슬님이 방장이 되어 팀채팅을 시작했어요!\r\n"
            "[김예슬] [오후 8:35] 안녕하세요!\r\n"
            "성경 읽기 방입니다~!\r\n"
            "\r\n"
            "--------------- 2026년 2월 2일 월요일 ---------------\r\n"
            "[김예슬] [오전 7:53] 🗓️ 2/2 창세기 1-4\r\n"
            "[🍯 이상호 99] [오전 8:06] 2/2 🐵\r\n"
            "[🍯 김신영 99 🦄] [오전 8:11] 2/2 🍡\r\n"
            "김예슬님이 이솔지 96님을 초대했습니다.\r\n"
            "[정주희] [오후 9:24] 2/2🍓\r\n"
        )
        rows = parse_txt(text)
        assert len(rows) == 5
        assert rows[0] == ("김예슬", "안녕하세요!\n성경 읽기 방입니다~!")
        assert rows[1] == ("김예슬", "🗓️ 2/2 창세기 1-4")
        assert rows[2] == ("🍯 이상호 99", "2/2 🐵")
        # 시스템 메시지가 이전 사용자 메시지의 멀티라인으로 연결됨
        assert rows[3] == (
            "🍯 김신영 99 🦄",
            "2/2 🍡\n김예슬님이 이솔지 96님을 초대했습니다.",
        )
        assert rows[4] == ("정주희", "2/2🍓")


class TestExtractChatMetaWindows:
    def test_윈도우_방이름_및_ISO_저장날짜_추출(self):
        text = (
            "📖26 성경일독 PART1🙏🏻 님과 카카오톡 대화\r\n"
            "저장한 날짜 : 2026-02-14 14:09:18\r\n"
        )
        meta = extract_chat_meta(text)
        assert meta["room_name"] == "📖26 성경일독 PART1🙏🏻"
        assert meta["saved_date"] == "2026/02/14-14:09"

    def test_윈도우_방이름_없음__None_반환(self):
        text = "저장한 날짜 : 2026-02-14 14:09:18\r\n"
        meta = extract_chat_meta(text)
        assert meta["room_name"] is None
        assert meta["saved_date"] == "2026/02/14-14:09"
