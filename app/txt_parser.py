...
# 저장 날짜 줄: "저장한 날짜 : 2026. 2. 9. 오전 10:50" 또는 24시간제 "저장한 날짜 : 2026. 3. 1. 08:11"
_SAVED_DATE_RE = re.compile(
    r"^저장한 날짜\s*:\s*(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})\.\s*(오전|오후)?\s*(\d{1,2}):(\d{2})"
)

# 사용자 메시지: "2026. 2. 2. 오전 7:33, 유저이름 : 메시지" 또는 24시간제 "2026. 2. 2. 20:21, 유저이름 : 메시지"
_USER_MSG_RE = re.compile(
    r"^\d{4}\.\s*\d{1,2}\.\s*\d{1,2}\.\s*(?:[오전후]+\s*)?\d{1,2}:\d{2},\s*(.+?)\s*:\s*(.*)"
)

# 시스템 메시지: "2026. 2. 1. 오후 8:26: 시스템텍스트" 또는 24시간제 "2026. 2. 1. 20:21: 시스템텍스트" (쉼표 없이 콜론)
